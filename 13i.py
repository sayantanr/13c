    ...
    if st.button("🚀 Solve 13C-MFA Fast", type="primary"):
        with st.status("Building EMU model and solving NLP...", expanded=True) as status:
            try:
                ...
                opts = {
                    'ipopt': {
                        'max_iter': max_iter, 'tol': tolerance, 'print_level': 0,
                        'linear_solver': linear_solver, 'jacobian_approximation': 'exact',
    ...
    if st.session_state.model and st.session_state.flux_result is not None:
        ...
        with col1:
            ...
            with st.spinner("Running pFBA..."):
                # FIX: Use safe_pfba with bound checking
            ...
            try:
                ...
                st.warning(f"FVA skipped: {str(e)[:80]}")

        with col2:
            st.subheader("Dashboard 16: KO Simulation")
            ko_gene = st.selectbox("Knockout Gene", [g.id for g in model.genes[:50]] if model.genes else ["None"])
            if st.button("Simulate KO") and ko_gene!= "None":
