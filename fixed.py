...
import casadi as ca
# Attempt to import IPOPT support; fallback to alternative solvers if unavailable.
_ipopt_available = True
try:
    # Creating a dummy NLPSOL with IPOPT to test availability
    x = ca.SX.sym('x', 1)
    nlp = {'x': x, 'f': x**2}
    ca.nlpsol('test', 'ipopt', nlp)
except Exception:
    _ipopt_available = False
    # Log the fallback internally (Streamlit will show a warning later)
    import warnings
    warnings.warn('IPOPT backend not available; defaulting to SQP method.', UserWarning)
...
# ==================== HELPER: SAFE BOUNDS ====================
def set_safe_bounds(rxn, v_mfa, factor=0.1):
    """Set bounds ensuring lb <= ub even for negative fluxes"""
    target = v_mfa
    lb = target * (1 - factor) if target >= 0 else target * (1 + factor)
    ub = target * (1 + factor) if target >= 0 else target * (1 - factor)
    # Ensure lb <= ub numerically
    lb, ub = min(lb, ub), max(lb, ub)
    # Clip to model bounds
    lb = max(lb, rxn.lower_bound)
    ub = min(ub, rxn.upper_bound)
    rxn.bounds = (lb, ub)

# Safe pFBA wrapper that never crashes the app
# Safe pFBA wrapper that never crashes the app
def safe_pfba(model, reactions_to_constrain=None, v_mfa=None):
    """pFBA with error handling - never crashes"""
    try:
        with model:
            if reactions_to_constrain and v_mfa is not None:
                for i, rxn in enumerate(reactions_to_constrain[:len(v_mfa)]):
                    set_safe_bounds(rxn, v_mfa[i], factor=0.1)
            sol = pfba(model)
            return sol, None
    except Infeasible:
        return None, "Model infeasible with MFA constraints"
    except Exception as e:
        return None, f"pFBA error: {str(e)[:100]}"
...
with st.sidebar:
    ...
    default_solver = "ipopt" if _ipopt_available else "sqpmethod"
    solver_choice = st.selectbox("NLP Solver", ["ipopt", "sqpmethod"], index=0 if _ipopt_available else 1, help="IPOPT requires external binaries. If unavailable, SQP will be used.")
...
with tab1:
    ...
    with col1:
        ...
        if st.session_state.model:
            ...
            st.dataframe(mets_df, width='stretch', height=400)
...
with tab2:
    ...
    if uploaded_mid:
        try:
            ...
            st.dataframe(mid_df.head(20), width='stretch')
            col1, col2 = st.columns(2)
            with col1:
            ...
            with col2:
                ...
                nC = 6
                C = np.array([[norm.pdf(i-j, 0.01) for j in range(nC+1)] for i in range(nC+1)])
...
# ==================== TAB 3: NLP SOLVER - OPTIMIZED ====================
with tab3:
    st.header("3. Nonlinear IPOPT Solver - OPTIMIZED")
    if not _ipopt_available and solver_choice == "ipopt":
        st.warning("IPOPT not detected. Switching to SQP method.")
        solver_choice = "sqpmethod"

    # Provide user guidance for installing IPOPT on Windows
    if not _ipopt_available:
        st.info("To install IPOPT on Windows, you can use Conda: `conda install -c conda-forge ipopt` and then install `cyipopt` via `pip install cyipopt`. Restart the app after installation.")

    if st.button("🚀 Solve 13C-MFA Fast", type="primary"):
        with st.status("Building EMU model and solving NLP...", expanded=True) as status:
            try:
                t0 = time.time()
                st.write("Step 1/4: Building NLP...")
                ...
                nlp = {'x': v, 'f': obj}
                st.write("Step 2/4: Configuring IPOPT...")
                opts = {
                    'ipopt': {
                        'max_iter': max_iter,
                        'tol': tolerance,
                ...
                status.update(label=f"Converged in {solver.stats()['iter_count']} iterations | {st.session_state.solve_time:.2f}s", state="complete")
            except Exception as e:
...
with tab4:
    ...
    if st.button("🎲 Run MCMC Sampling Fast"):
        with st.status(f"Running {mcmc_samples} MCMC samples...", expanded=True) as status:
            if st.session_state.flux_result is None:
                st.error("Solve NLP first")
                status.update(label="No fluxes", state="error")
            else:
                t0 = time.time()
                v0 = st.session_state.flux_result
                nv = len(v0)
                st.write("Step 1/3: Pre-generating random numbers...")
                ...
                status.update(label=f"Collected {len(st.session_state.mcmc_samples)} samples in {t_elapsed:.2f}s", state="complete")
...
# ==================== TAB 5: FBA INTEGRATION - FIXED ====================
with tab5:
    st.header("5. COBRA-FBA Integration & Knockout Simulation - CRASH-PROOF")
    try:
        if st.session_state.model and st.session_state.flux_result is not None:
            ...
            with col1:
                ...
                if st.button("Run / Retry pFBA", key="pFBA_tab5"):
                    with st.spinner("Running pFBA..."):
                        pfba_sol, err = safe_pfba(model, model.reactions, v_mfa)
                        if pfba_sol:
                            ...
                            st.success("pFBA completed successfully")
                        else:
                            st.error(f"pFBA failed: {err}")

                st.subheader("Graph 15: FVA Ranges")
                try:
                    with st.spinner("Running FVA..."):
                    ...
                    st.error(f"FVA failed: {str(e)[:100]}")
            ...
            with col2:
                ...
                # Safely get gene list
                gene_options = [g.id for g in model.genes[:50]] if model.genes else []
                if not gene_options:
                    st.info("No genes available for knockout.")
                else:
                    ko_gene = st.selectbox("Knockout Gene", options=gene_options, key="ko_gene_select")
                    if st.button("Simulate KO", key="ko_sim") and ko_gene != "None":
                        with st.spinner(f"Knocking out {ko_gene}..."):
                            try:
                                with model:
                                ...
                                st.error(f"KO failed: {str(e)[:100]}")
                            finally:
                                # Ensure any temporary knockout state is cleared; 'with model' restores automatically
                                pass
        else:
            st.info("Load a model and run the NLP solver first.")
    except Exception as e:
        st.error(f"Unexpected error in Tab 5: {str(e)}")
